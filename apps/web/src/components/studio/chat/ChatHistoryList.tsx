...
export const ChatHistoryList: React.FC<ChatHistoryListProps> = ({
	...
	const [activeTab, setActiveTab] = React.useState<"threads" | "media">(
		"threads",
	);
	...
	const deleteMutation = useMutation({
		...trpc.agent.deleteThread.mutationOptions(),
		onSuccess: () => {
			queryClient.invalidateQueries(
				trpc.agent.getThreads.queryFilter({ limit: 50 }),
			);
			if (activeThreadId === deletingThreadId) {
				onSelectThread(""); // Clear active thread if it was deleted
			}
	...
	const handleDelete = (e: React.MouseEvent, id: string) => {
		e.stopPropagation();
		if (deletingThreadId === id) {
			deleteMutation.mutate({ id });
		} else {
			setDeletingThreadId(id);
			// Reset after 3 seconds if not confirmed
			setTimeout(
				() => setDeletingThreadId((prev) => (prev === id ? null : prev)),
				3000,
			);
		}
	};
	...
	return (
		...
		>
			...
			<div className="flex border-b border-border px-2 bg-muted/30">
				{(["threads", "media"] as const).map((tab) => (
					<button
						key={tab}
			...
			{activeTab === "threads" ? (
				...
				<div className="flex-1 overflow-y-auto scrollbar-swiss p-0.5">
					{workspaceId ? (
						<ChatMediaGrid workspaceId={workspaceId} />
					) : (
