...
import { IsometricServerBlade } from "./IsometricServerBlade";
import { LensApertureSchematic } from "./LensApertureSchematic";
...
export const SystemSpecs = () => {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			setActiveSchematic((prev) => (prev + 1) % 2);
		}, 5000); // Cycle every 5 seconds
		return () => clearInterval(interval);
	}, []);

	const renderSchematic = () => {
		switch (activeSchematic) {
			case 0:
				return <IsometricServerBlade />;
			case 1:
				return <LensApertureSchematic />;
			default:
				return <IsometricServerBlade />;
		}
	};
				...
				{SPECS.map((spec, index) => (
					...
					>
						...
						<div className="flex items-center justify-between pl-8 border-l border-neutral-100 dark:border-neutral-900 ml-2 py-1">
							...
							<span
								className={`font-mono text-[10px] ${spec.status === "ONLINE" || spec.status === "ACTIVE" ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}
							>
								[{spec.status}]
							</span>
						</div>
			...
			>
				<div className="flex gap-1">
					{[1, 2, 3, 4].map((i) => (
						<div
							key={i}
							className={`w-1 h-2 ${i === 1 ? "bg-emerald-500" : "bg-neutral-200 dark:bg-neutral-800"} animate-pulse`}
