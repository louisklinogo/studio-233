...
export async function updateWorkspaceBrandDNA(
	...
	await prisma.workspace.update({
		...
		data: {
			brandProfile: dna,
			brandSummary: dna.visualStyle.vibe,
			updatedAt: new Date(),
		},
	});
...
export async function pdfToImages(filePath, maxPages = 50) {
	const images = [];
	try {
...
async function processWithLlamaParse(options) {
...
async function processWithGeminiVision(options) {
	const apiKey =
